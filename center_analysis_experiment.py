...
DEFAULT_OUTPUT = "outputs/center_analysis/twin_center_analysis.json"
DEFAULT_SUMMARY_OUTPUT = "outputs/center_analysis/twin_center_analysis_summary.json"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run canonical twin-center analysis for an inclusive range of n values."
    )
    ...
    parser.add_argument(
        "--summary-only",
        action="store_true",
        help="Write only summary data and omit per-center records.",
    )
    parser.add_argument(
        "--output",
        default=None,
...
def main():
    ...
    if args.summary_only:
        records = []
        summary = summarize_center_range(args.start_n, args.end_n)
    else:
        records = analyze_center_range(args.start_n, args.end_n)
    ...
    print(
        f"Analyzed {summary['center_count']} centers; "
        f"found {summary['twin_prime_count']} twin-prime centers."
    )
